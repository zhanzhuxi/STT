...
class Dual_Contrastive_Loss(nn.Module):
    def __init__(self, args=None):
        super().__init__()
        self.logit_scale_inter = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))    # 定义一个可训练的温度缩放参数
        # 通过训练，这个参数可以自动调整以获得更好的对比学习效果。

        self.NCE_video = NCE()
        self.NCE_text = NCE()

    # cls_video和cls_text是分别前缀在视频特征和文本特征最前面的用于表示整个视频或文本的整体表示
    def forward(self, contrastive_pairs):
        ...
        cls_video = F.normalize(cls_video.squeeze(1), dim=1) #[B, C]    torch.Size([4, 128])
        cls_text = F.normalize(cls_text.squeeze(1), dim=1) #[B, C]      torch.Size([4, 128])

        # cosine similarity as logits
        logits_per_video = self.logit_scale_inter.exp() * cls_video @ cls_text.t() #[B, B]
        logits_per_text = logits_per_video.t() #[B, B]  文本对视频做个转置即可
...
class SupConLoss_clear(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        features = F.normalize(features.squeeze(1), dim=1)  # [B, C]    torch.Size([4, 128])
        device = (torch.device('cuda') if features.is_cuda else torch.device('cpu'))
        batch_size = features.shape[0]

        labels = [int(label) for label in labels]
        labels = torch.tensor(labels).view(-1, 1).to(device)
        labels = labels.long()
